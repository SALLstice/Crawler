...
class Trap:
    def __init__(self, hidden, damage):
        self.hidden = hidden
        self.damage = damage
...
class Room:
    def __init__(self, ID, doors, monsters, treasures, traps):
        self.ID = ID
        self.doors = doors
...
def buildRoom(count):
    ...
    roomBuild = Room(count, do, mo, tre, tra)
    return roomBuild
...
def move(pos, dir):
    if sum(mon.health for mon in dungeon[pos[0], pos[1]].monsters) <= 0:
